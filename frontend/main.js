...
function cacheEntry() {
    ...
    else entry = { lines: [], offset: -1, id: "", done: false };
    cache.set(key, entry);
...
function connect(locate) {
    ...
    if (state.file.all) {
        ...
        if (entry.offset >= 0) {
            p.set("offset", String(entry.offset));
            if (entry.id) p.set("id", entry.id); // prove the offset belongs to this very file
        }
    ...
    src.addEventListener("id", e => {
        if (entry) entry.id = e.data; // the file identity the cached offset belongs to
    });
    src.addEventListener("reset", () => {
        // The file shrank or was replaced: everything shown and cached is
        // invalid. Clear both — the stream now carries the new file from its
        // start (a fresh "id" frame follows with its identity).
        if (entry) { entry.lines = []; entry.offset = -1; entry.id = ""; }
        logview.clear();
    });
    ...
    src.onerror = () => {
        // EventSource auto-reconnect would reuse the URL built at connect
        // time, whose offset is stale by now — the server would re-send
        // everything since and the view would append it twice. Reconnect
        // ourselves instead: connect() rebuilds the request from the current
        // cache state.
        src.close();
        if (state.source !== src) return; // a superseded stream's late error
        state.source = null;
        setStatus("reconnecting…");
        const seq = findSeq;
        setTimeout(() => { if (findSeq === seq) connect(); }, 2000);
    };
}
